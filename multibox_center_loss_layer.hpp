...
class MultiBoxCenterLossLayer : public LossLayer<Dtype> {
 public:
  explicit MultiBoxCenterLossLayer(const LayerParameter& param)
  ...
  virtual inline const char* type() const { return "MultiBoxCenterLoss"; }
 ...
 protected:
  ...
  float loc_weight_;
  float center_loss_weight_; //weights for penalizing the center_loss
  ...
  // The center confidence loss layer.
  shared_ptr<Layer<Dtype> > conf_center_loss_layer_;
  ...
  // bottom vector holder used in Forward function.
  vector<Blob<Dtype>*> conf_center_bottom_vec_;
  // top vector holder used in Forward function.
  vector<Blob<Dtype>*> conf_center_top_vec_;
  // blob which stores the confidence prediction.
  Blob<Dtype> conf_pred_;
  // blob which stores the confidence prediction used by center loss.
  Blob<Dtype> conf_center_pred_;
  // blob which stores the corresponding ground truth label.
  Blob<Dtype> conf_gt_;
  // confidence loss.
  Blob<Dtype> conf_loss_;
  // confidence center loss.
  Blob<Dtype> conf_center_loss_;

  MultiBoxLossParameter multibox_loss_param_;
  int num_classes_;
  int center_features_;
